...
function drawWatermark(doc: PdfDoc): void {
  ...
  doc.fontSize(42).fillColor('#e5e7eb').text(WATERMARK, 40, height / 2 - 20, {
    width: width + 200,
    align: 'center',
  });
  doc.restore();
...
export function renderRiskAssessmentPdf(ctx: RiskAssessmentContext): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.moveDown(0.2).fontSize(10).fillColor('#6b7280').text(
      'GPSR — Regolamento (UE) 2023/988 · Art. 9(5) · Categoria: giocattoli',
    );
    doc.moveDown(0.8);
    ...
    if (ctx.classificationConfidence != null) {
      doc.text(`Confidenza classificazione AI: ${(ctx.classificationConfidence * 100).toFixed(0)}%`);
    }
    ...
    doc.moveDown(1).fontSize(8).fillColor('#9ca3af').text(
      `Template: ${ctx.templateId} v${ctx.templateVersion} · Generato: ${ctx.generatedAt}`,
      { align: 'right' },
    );
